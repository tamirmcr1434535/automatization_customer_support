...
C) DELETE ACCOUNT signals (アカウント削除, delete my account, 계정 삭제,
   ...
   → ALWAYS → DELETE_ALL_DATA. The bot must NEVER auto-reply or auto-cancel
     a ticket where the customer asked to delete their account. A human always
     handles account-deletion requests because they involve GDPR/data-removal
     scope decisions that go beyond a subscription cancellation.
   → This applies even when the customer ALSO mentions billing, charges, or
     a subscription — "delete my account, I'm being charged" still goes to
     DELETE_ALL_DATA (NOT TRIAL_CANCELLATION). The previous rule that mapped
     delete+billing → TRIAL_CANCELLATION caused 106847 (KR "계정 탈퇴 해주세요")
     to be auto-cancelled with a reply when the customer wanted account
     deletion, not just trial cancellation.
   → If delete account + explicit refund demand (返金, refund, reembolso,
     hoàn tiền, etc.) → REFUND_REQUEST (also a human-escalation intent).
   → Korean: "계정 탈퇴 해주세요" / "회원 탈퇴" / "탈퇴해주세요" = DELETE_ALL_DATA.
     Korean 탈퇴 by itself can mean "cancel subscription", but combined with
     계정 (account) or 회원 (membership) it means "delete account".
