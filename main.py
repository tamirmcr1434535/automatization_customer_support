...
#
# We populate it from WooCommerce — primary source is customer.meta_data
# 'country' (full English name like "Japan"), with billing.country as a
# fallback. Full names are mapped to the field's ISO-2 tag via the lazy
# name→tag dict below. Empty / unrecognised values silently skip.
_ZENDESK_COUNTRY_FIELD_ID = os.getenv(
    "ZENDESK_COUNTRY_FIELD_ID", "18169250913436",
).strip()

# Lazy cache of the Country field's name→tag map (e.g. "Japan" → "jp").
# Loaded on first set call by fetching /api/v2/ticket_fields/{id} — 241
# options, one network call per cold start, then in-memory forever.
_COUNTRY_NAME_TO_TAG: dict[str, str] | None = None


def _load_country_name_to_tag() -> dict[str, str]:
    """Fetch Country field options from Zendesk once; return a
    case-insensitive {name: tag} map (e.g. {"japan": "jp"}). Returns {}
    on any failure so callers silently skip instead of crashing."""
    global _COUNTRY_NAME_TO_TAG
    if _COUNTRY_NAME_TO_TAG is not None:
        return _COUNTRY_NAME_TO_TAG
    try:
        field = zendesk.get_ticket_field(int(_ZENDESK_COUNTRY_FIELD_ID))
        opts = (field or {}).get("custom_field_options") or []
        _COUNTRY_NAME_TO_TAG = {
            str(o.get("name", "")).strip().lower():
            str(o.get("value", "")).strip().lower()
            for o in opts
            if o.get("name") and o.get("value")
        }
        log.info(
            f"Country field options loaded: {len(_COUNTRY_NAME_TO_TAG)} entries"
        )
    except Exception as e:
...
def _set_country_for_ticket(ticket_id: str, country: str) -> None:
    """
    Set the Zendesk Country custom field.

    Accepts either an ISO-2 code ("JP" / "jp") or a full English name
    ("Japan", "Korea, Republic of"). Full names are converted via the
    Zendesk Country field options (lazy-loaded once per process).

    Silently skipped when the field id is unset, the value is empty, or
    no matching Zendesk option exists. Never raises — country is a
    reporting aid, not part of the cancellation guarantee.
    """
    ...
    if len(raw) == 2 and raw.isalpha():
        ...
        name_to_tag = _load_country_name_to_tag()
        value = name_to_tag.get(raw.lower(), "")
        if not value:
            log.warning(
                f"[{ticket_id}] Country {country!r} not in Zendesk field options — skip"
            )
            return
...
def _process(ticket_id: str) -> dict:
    ...
    if cancel_status == "wc_lookup_error":
        ...
        # Agent-handoff race guard: WC lookup can block for 60-180s. While
        # we were waiting, a human agent may have:
        #   1. Replied publicly — caught by last_public_comment_is_from_agent.
        #   2. Just SOLVED the ticket (no public reply, maybe just an
        #      internal note + status change) — caught by re-fetching
        #      ticket status. Vova in the second korobocle.s incident
        #      solved without a public reply; the public-comment check
        #      alone missed it, the bot then re-opened the ticket and
        #      overwrote Vova's tags.
        # Either signal means "human is on it" — abort all writes.
        ticket_now = zendesk.get_ticket(ticket_id)
        if ticket_now and ticket_now.get("status") in ("solved", "closed"):
            log.info(
                f"[{ticket_id}] Status became "
                f"{ticket_now.get('status')!r} during WC lookup — "
                "skipping WC error write (human resolved it)"
            )
            result["status"] = "skipped_agent_already_replied"
            return result
